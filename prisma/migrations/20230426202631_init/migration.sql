...
CREATE TABLE `Order` (
    ...
    `updatedAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    `userId` VARCHAR(191) NOT NULL,
    `paymentMethod` ENUM('cash', 'visaCard', 'paypel') NOT NULL DEFAULT 'visaCard',
...
-- CreateTable
CREATE TABLE `BookedPlace` (
    `id` VARCHAR(191) NOT NULL,
    `startsAt` DATETIME(3) NOT NULL,
    `numberOfHours` INTEGER NOT NULL DEFAULT 2,
    `userId` VARCHAR(191) NOT NULL,
    `createdAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    `updatedAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
...
-- AddForeignKey
ALTER TABLE `BookedPlace` ADD CONSTRAINT `BookedPlace_userId_fkey` FOREIGN KEY (`userId`) REFERENCES `User`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;
